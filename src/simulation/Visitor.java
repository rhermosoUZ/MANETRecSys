...
public class Visitor {
	...
	void move(Grid<Object> grid) {
		...
		processStates();
		moveTowardsOnSpace(targetPoint);

		// save local knowledge of visitor (= all data (zscores) he collected from neighbors)
	...
	public void processStates() {
		if (this.uid == Visitor.traceVisitor)
			System.out.println("State --> " + this.state + " Target Point -> " + this.targetPoint + " - Current location " + grid.getLocation(this));
		
		if (state == State.arrivedAtNewExhibit) { 	// the visitor has just arrived a new exhibit => draw sojourn time
...
//			int steps = drawSojournTimeAndSaveResult();
		...
		if (state == State.goToNextExhibit ) {
			...
			if (hasVisitedAllExhibits()) { // special case: visitor has visited everything: goto exit
				...
				targetExhibit = getNewExhibit(); 					// get new exhibit
...
//				}
	...
	private int drawSojournTimeAndSaveResult() {  					// invoked when arriving at new exhibit
		...
		int sojournTime = behavior.drawGaussianFromInterestRank(rankInInterest); 
...
//			 md.getEvaluationResults().get(exhibitId).getLocalPrediction() + " sojourn:" +
//			 currentSojournTime);
			md.getEvaluationResults().get(exhibitId).setMeasuredSojournTime(sojournTime);
			if (this.uid == Visitor.traceVisitor && Configuration.followRecommendedVisits) {
				System.out.println("[RECOMMENDED FOR VISITOR " + this.id() + "] type " + exhibitType
						+ "; interest level:" + rankInInterest + "   EXHIBIT:" + exhibitId
						+ "   SOJOURN TIME: " + sojournTime);
			}
		} else {
			if (this.uid == Visitor.traceVisitor && Configuration.followRandomVisits) {
				System.out.println("[RANDOM VISIT] for visitor " + this.id() + "  tick:" + tick
				// + " of type " + exhibitType
				// + " interest level:" + interestProfile.getMeanValueForInterestType(exhibitType)
	...
	private Exhibit getNewExhibit() {
		int recommendation = 0;
		double tick = RunEnvironment.getInstance().getCurrentSchedule().getTickCount();
		RecommendationResult result = md.getRecommendation();

		this.getMobileDevice().printWhomImetWith();
		
		if (result != null) {
			recommendation = result.itemID();
			md.getEvaluationResults().put(result.itemID(), new EvaluationResult(uid, result.itemID(), result.predictionAsInt(),
					result.randomResult(), result.globalResult(), tick, LocalDateTime.now(), false));
		...
		if (recommendation != 0) {
			for (Exhibit e : targets) {
				if (e.getId() == recommendation && !md.hasVisited(e.getId())) {
					exhibit = e;
					break;
				}
			}
...
//					if (this.uid == this.traceVisitor) {
					if (this.uid == Visitor.traceVisitor && Configuration.printRecommendationResults) {
						System.out.println("[RANDOM VISIT] Visitor " + this.id() + "]  item:"  + e.getId() + " of type: " + e.getType());
...
//				System.out.println("[DISTANCE] -> " + pt + " and " + grid.getLocation(this)  + distance(pt, grid.getLocation(this)));
			...
			if (distance(pt, grid.getLocation(this)) <= this.behavior.velocity()) {
				...
				if (this.uid == Visitor.traceVisitor && Configuration.printRecommendationResults)
					System.out.println("... " + this.uid + " new location is [" + (int) myPoint.getX() + ", " + (int) myPoint.getY() + "]");
			}
		}
	}
